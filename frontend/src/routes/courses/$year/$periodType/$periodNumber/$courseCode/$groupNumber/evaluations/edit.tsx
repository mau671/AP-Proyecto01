...
import { ArrowLeft, CalendarIcon, Upload, File as FileIcon, X, Sparkles, Plus, Trash, Pencil, ChevronUp, ChevronDown, User, Shuffle, Check, Users, GripVertical } from 'lucide-react'
...
function EditEvaluationPage() {
  ...
  // Parse initial date from "DD/MM/YYYY HH:mm" format if possible
  const getInitialDate = () => {
    if (item.dueDate) {
      ...
      if (parts.length >= 5) {
        ...
        return new Date(Number(parts[2]), Number(parts[1]) - 1, Number(parts[0]), 23, 55)
      }
    }
    return undefined
  }
  ...
  // Form States
  const [evalDate, setEvalDate] = useState<Date | undefined>(getInitialDate())
  const [hasRubric, setHasRubric] = useState(item.rubric)
  const [allowLate, setAllowLate] = useState(item.allowLate)
  const [peoplePerGroup, setPeoplePerGroup] = useState(item.peoplePerGroup)
  const [evalScore, setEvalScore] = useState(item.score.max)
  const [catComboVal, setCatComboVal] = useState<string>(group.title)
  ...
  // Helper to parse groups and students from item.members
  const getInitialGroupsAndStudents = () => {
    const allStudents = [
    ...
    if (item.members && item.members.length > 0) {
      const isFormatted = item.members.some(m => m.includes(':'))
      if (isFormatted) {
        item.members.forEach((m, idx) => {
          const colonIdx = m.indexOf(':')
          if (colonIdx !== -1) {
            const name = m.substring(0, colonIdx).trim()
            const studentsStr = m.substring(colonIdx + 1)
            const students = studentsStr.split(',').map(s => s.trim()).filter(s => allStudents.includes(s))
            students.forEach(s => assignedSet.add(s))
            groupsList.push({
        ...
        // Fallback: use item.members as group names, and pre-assign some students
        let studentIdx = 0
        item.members.forEach((gName, idx) => {
          const students: string[] = []
          for (let i = 0; i < 2 && studentIdx < allStudents.length; i++) {
            students.push(allStudents[studentIdx])
            assignedSet.add(allStudents[studentIdx])
            studentIdx++
          }
          groupsList.push({
            id: `group-${idx}-${Date.now()}`,
    ...
    const unassigned = allStudents.filter(s => !assignedSet.has(s))
    return { initialGroupsList: groupsList, initialUnassigned: unassigned }
  }

  const { initialGroupsList, initialUnassigned } = getInitialGroupsAndStudents()

  const [unassignedStudents, setUnassignedStudents] = useState<string[]>(initialUnassigned)
  const [groups, setGroups] = useState<Group[]>(initialGroupsList)
  ...
  const [attachedFile, setAttachedFile] = useState<File | null>(
    item.name === 'Tarea 1' ? new File([], 'guia_practica_tarea1.pdf', { type: 'application/pdf' }) : null
  )
  ...
  const handleSaveEval = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const updatedEval: EvaluationItem = {
      ...item,
    ...
    // If moving to another category, remove from old, push to new
    if (groupIndex === gIndex) {
      newGroups[gIndex].items[itemIndex] = updatedEval
    } else {
      newGroups[groupIndex].items = newGroups[groupIndex].items.filter((_, idx) => idx !== itemIndex)
      newGroups[gIndex].items.push(updatedEval)
    }
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {attachedFile.size > 0 ? `${(attachedFile.size / (1024 * 1024)).toFixed(2)} MB` : 'PDF'}
                      </p>
                    </div>
                  </div>
                  <Button
