                          ...
                          <div className="grid gap-0 border-t border-border md:grid-cols-2">
                            ...
                            <div className="px-3 pb-3 pt-1 md:p-3">
                              ...
                              <div className="pl-2">
                                ...
                                {item.feedbackFiles?.length ? <div className="mb-2"><p className={detailLabelClass}>Archivos de retroalimentación:</p><div className="space-y-1 pt-1">{item.feedbackFiles.map((file) => <div key={file.name} className="flex items-center gap-2 text-sm text-muted-foreground"><p className="min-w-0 flex-1 truncate">{file.name}</p><button type="button" className="shrink-0 cursor-pointer rounded-md p-1 hover:bg-accent hover:text-foreground" aria-label={`Descargar ${file.name}`}><DownloadIcon className="size-4" /></button></div>)}</div></div> : null}
