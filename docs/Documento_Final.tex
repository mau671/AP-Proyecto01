...
\begin{figure}[H]
\centering
\SafeImage[1\textwidth]{Entregable 05 - Estructura de desglose del trabajo.imgs.pdf}
...
El presente apartado aborda la planificación temporal de la \textit{Plataforma Universitaria Integrada}, detallando la distribución estratégica de las actividades a lo largo del ciclo de vida de la iniciativa. Esta estructuración es fundamental porque permite establecer un marco de trabajo ordenado donde se delimitan los plazos para el desarrollo técnico del prototipo interactivo y se definen con precisión las fechas clave para los procesos de validación por parte de la dirección. Mediante esta organización secuencial, el equipo de gestión garantiza que los recursos se utilicen de manera eficiente, optimizando la asignación de esfuerzos humanos y financieros para evitar cuellos de botella que puedan comprometer la entrega final. 

El cronograma oficial del proyecto fue elaborado íntegramente utilizando la herramienta Microsoft Project, cuya capacidad analítica permite enlazar dependencias lógicas complejas, calcular rutas críticas de manera automática y consolidar múltiples capas de información financiera y temporal en un solo repositorio. De esta manera, el plan de trabajo estructurado trasciende el formato de un simple calendario para convertirse en un instrumento de monitoreo y control dinámico que guía todas las fases de ejecución del prototipo web.

\subsection{Configuración general del cronograma}
...
\subsection{Evidencia visual del cronograma}

Para garantizar la correcta legibilidad de la planificación temporal y sus componentes gráficos, a continuación se adjuntan las páginas que componen el cronograma general del proyecto, donde se pueden observar las barras del calendario, las dependencias entre las tareas, la distribución temporal de cada fase y la ruta crítica señalada por el software. Cabe destacar que el archivo nativo completamente interactivo y editable se encuentra disponible de manera adjunta bajo el nombre \textbf{Entregable 06 - Cronograma del Proyecto.mpp}, permitiendo así a los interesados revisar la configuración original y los datos consolidados directamente en Microsoft Project.

\includepdf[pages=-, fitpaper=true]{Entregable 06 - Cronograma.imgs.pdf}

\subsection{Estructura por fases}
...
\subsection{Tareas, subtareas e hitos}
...
\subsection{Dependencias y predecesoras}
...
\subsection{Ruta crítica del proyecto}
...
Además de los recursos humanos, el cronograma considera recursos de costo: Software y herramientas, Hosting o infraestructura, Hardware y materiales, Servicios operativos y Contingencia. Estos recursos permiten representar costos del proyecto que no se calculan como horas de trabajo, sino como rubros presupuestarios.
...
\begin{widepage}
\subsection{Tabla del diagrama de Gantt}

La vista tabular del diagrama de Gantt que se expone a continuación abarca desde la denominación de cada tarea hasta los costos asociados, estructurando la información en bloques de trabajo sucesivos para facilitar su lectura. De esta manera, cada segmento expone de forma consolidada métricas fundamentales para el control del proyecto, tales como la duración estimada, las fechas programadas, las dependencias predecesoras, la asignación de recursos y los costos fijos totales vinculados a cada actividad.
...
\caption{Inicio y planificación base}
\end{table}
...
\caption{Planificación complementaria}
\end{table}
...
\caption{Ejecución del prototipo}
\end{table}
...
\caption{Supervisión, cierre y rubros adicionales}
\end{table}



\subsection{Hoja de recursos}

La hoja de recursos muestra los recursos humanos, los recursos de costo y la forma en que se acumulan dentro del proyecto. Esta vista permite verificar que el cronograma incluye recursos de trabajo y recursos de costo.

\begin{table}[H]
\centering
\caption{Hoja de recursos utilizada en Microsoft Project}
{\tiny
\renewcommand{\arraystretch}{1.18}
\setlength{\tabcolsep}{1.0pt}
\begin{tabular}{L{4.5cm} L{1.55cm} L{1.25cm} L{1.25cm} L{3.1cm} L{1.35cm} L{1.75cm} L{1.75cm} L{1.45cm} L{1.55cm} L{1.65cm}}
\toprule
...
\subsection{Responsabilidades principales por rol}

La siguiente tabla muestra cómo se relacionan los roles del organigrama con los componentes clave del proyecto, de esta manera se define la responsabilidad de cada parte.

\begin{table}[H]
\centering
\small
\caption{Responsabilidades principales por rol}\label{tab:roles-entregables}
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\textwidth}{p{4.3cm} Y}
\toprule
\textbf{Rol} & \textbf{Componentes o actividades relacionadas} \\
\midrule
...
La presente sección documenta el prototipo funcional desarrollado para la plataforma universitaria, el cual integra los principales procesos académicos, administrativos y financieros de la institución en una experiencia de usuario fluida y moderna.

Cabe mencionar que la interfaz cuenta con adaptabilidad visual, por lo cual la primera vista (el panel principal del estudiante) se exhibe tanto en modo oscuro como en modo claro. Para el resto de las pantallas, se utiliza exclusivamente el modo claro para mantener la consistencia en el formato del documento.

\subsection{Arquitectura y consideraciones técnicas}

Para la construcción del prototipo funcional se optó por una arquitectura basada en Single Page Application (SPA), la cual garantiza transiciones rápidas y sin recargas de página, mejorando drásticamente la experiencia del usuario. A nivel de tecnologías, el proyecto fue desarrollado utilizando el framework React en su versión 19 y Vite como herramienta de construcción principal, empleando TypeScript para asegurar un tipado estricto que reduce la probabilidad de errores durante la integración de datos simulados, junto a TanStack Router para manejar la navegación interna.

En cuanto al diseño visual, la interfaz se implementó utilizando Tailwind CSS en conjunto con la biblioteca de componentes prefabricados de shadcn/ui, lo que permitió un desarrollo ágil y altamente personalizado, facilitando la creación de un sistema de diseño consistente con soporte nativo para el modo claro y oscuro. Para la disposición de los elementos y la estructura de navegación, el equipo tomó una fuerte inspiración en plataformas consolidadas del entorno universitario, particularmente el TEC Digital, pero aplicando principios modernos de usabilidad e interactividad para resolver los problemas comunes de saturación visual.

\subsection{Portal estudiantil}

El portal estudiantil constituye el punto de acceso central mediante el cual los alumnos pueden tanto visualizar su información académica como navegar hacia las diferentes herramientas de la universidad. Durante su diseño, se dio prioridad a mostrar únicamente los indicadores clave con el fin de evitar sobrecargar al usuario de información innecesaria, aunque a futuro este panel podría enriquecerse mediante la incorporación de widgets personalizables que brinden mayor flexibilidad.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/01_student_home_dark.png}
    \caption{Panel principal del estudiante (modo oscuro).}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/01_student_home_light.png}
    \caption{Panel principal del estudiante (modo claro).}
\end{figure}

\subsubsection{Perfil e historial académico}

En esta sección, el estudiante puede gestionar la configuración de su cuenta y consultar un registro histórico consolidado de sus calificaciones pasadas, así como un resumen de sus promedios.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/02_student_profile.png}
    \caption{Perfil del estudiante y configuración de cuenta.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/19b_student_academic_history.png}
    \caption{Registro del historial académico y consolidado de promedios dentro del perfil.}
\end{figure}

\subsubsection{Matrícula de cursos}

Este apartado permite a los estudiantes seleccionar los cursos disponibles para el período activo y formalizar su inscripción académica de manera intuitiva a través de un flujo estructurado paso a paso, integrando un resumen en tiempo real de los créditos matriculados para agilizar la toma de decisiones sin necesidad de salir de la pantalla.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/03_student_enrollment.png}
    \caption{Proceso de matrícula y selección de cursos.}
\end{figure}

\subsubsection{Consulta de horarios}

A través de esta vista, los alumnos pueden visualizar de forma interactiva la distribución semanal de sus clases y compromisos académicos, apostando por un diseño de calendario limpio que permite identificar fácilmente la carga lectiva para que en futuras actualizaciones se ofrezca sincronización bidireccional con calendarios externos en la nube y alertas integradas en la plataforma.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/04_student_schedule.png}
    \caption{Horario de clases interactivo.}
\end{figure}

\subsection{Portal financiero}

El portal financiero centraliza el estado de cuenta del estudiante facilitando la revisión de deudas y la gestión de transacciones, con el fin de brindar una interfaz que ofrezca total transparencia sobre los diferentes rubros y conceptos cobrados por la institución.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/05_student_finance_account.png}
    \caption{Estado de cuenta general.}
\end{figure}

\subsubsection{Pagos de matrícula e historial}

Esta sección permite ejecutar el pago de obligaciones pendientes y consultar el archivo histórico de transacciones realizadas, dejando abierta la posibilidad de integrar de forma directa pasarelas de pago externas que faculten la cancelación de rubros en tiempo real junto con el envío de comprobantes o recordatorios automáticos por correo electrónico.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/06_student_finance_payment.png}
    \caption{Módulo para realizar pagos pendientes.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/07_student_finance_history.png}
    \caption{Historial de pagos completados.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/08_student_finance_other_charges.png}
    \caption{Sección de otros cobros y cargos administrativos.}
\end{figure}

\subsubsection{Aulas virtuales y consulta de calificaciones}

Cada curso matriculado cuenta con un entorno virtual propio donde el estudiante puede interactuar con el material didáctico, revisar el calendario específico y consultar las calificaciones detalladas asignadas por el profesor.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/09_student_course_home.png}
    \caption{Página principal del curso desde la vista del estudiante.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/10_student_course_calendar.png}
    \caption{Calendario específico del curso.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/11_student_course_documents.png}
    \caption{Repositorio de documentos y materiales.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/19a_student_course_evaluations.png}
    \caption{Vista de calificaciones detalladas por curso para el perfil de estudiante.}
\end{figure}

En cuanto a la vista del estudiante para la asistencia, se buscó evitar el diseño de una página estática tradicional, por lo que se aprovechó la misma disposición visual de la interfaz del docente para integrar un editor de apuntes que faculta a cada alumno a guardar notas de la clase directamente en la plataforma, abriendo la posibilidad futura de compartir estas anotaciones con los compañeros de grupo e incluso integrar inteligencia artificial para recibir sugerencias de redacción.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/12_student_course_attendance.png}
    \caption{Registro de asistencia interactivo con toma de apuntes.}
\end{figure}

\subsection{Portal docente}

El portal docente proporciona las herramientas necesarias para la administración de las aulas, el registro de asistencia y la creación de diferentes tipos de evaluaciones para dar un seguimiento adecuado al progreso de los estudiantes durante todo el ciclo lectivo.

\subsubsection{Gestión de cursos}

El panel de control del docente ofrece un acceso rápido a todos los cursos impartidos durante el ciclo actual, permitiendo gestionar el contenido y las configuraciones de cada aula virtual de manera ágil y centralizada.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/13_teacher_home.png}
    \caption{Panel de control del docente con los cursos impartidos.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/15_teacher_course_home.png}
    \caption{Página principal del curso desde la perspectiva del profesor.}
\end{figure}

\subsubsection{Registro de asistencia}

En la pestaña de asistencia, la vista del docente cuenta con un selector de clase en la zona izquierda y un gestor interactivo en la derecha que permite registrar rápidamente la presencia de cada estudiante, una funcionalidad que podría expandirse posteriormente para soportar múltiples lecciones dentro de una misma sesión junto a un registro específico de llegadas tardías.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/20_teacher_course_attendance.png}
    \caption{Registro y control de asistencia por parte del docente.}
\end{figure}

\subsubsection{Registro de calificaciones y evaluaciones}

Esta subsección dota a los instructores de mecanismos integrales para diseñar instrumentos de evaluación, asignar puntajes y revisar entregas de los estudiantes, preparando el terreno para implementar funcionalidades más avanzadas como rúbricas de calificación interactivas o soporte para la revisión entre pares.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/16_teacher_course_eval_new_group.png}
    \caption{Creador de evaluaciones grupales e individuales.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/17_teacher_course_gaap_new.png}
    \caption{Formulario para creación de evaluaciones de tipo guía de aprendizaje autónomo.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/18_teacher_course_gaap_review.png}
    \caption{Pantalla de revisión de entregas y respuestas de evaluaciones.}
\end{figure}

\subsection{Portal administrativo}

El panel de administración está concebido para que el personal autorizado gestione integralmente los catálogos de usuarios y toda la estructura académica de la universidad, lo que en el futuro podría agilizarse aún más mediante la incorporación de mecanismos para la importación masiva de datos desde hojas de cálculo y la inclusión de una bitácora de auditoría detallada.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/21_admin_home.png}
    \caption{Panel principal de administración de la plataforma.}
\end{figure}

\subsubsection{Gestión de estudiantes y docentes}

Mediante el desarrollo de interfaces de mantenimiento robustas, se garantiza la integridad de los registros al momento de incorporar y modificar perfiles del cuerpo estudiantil y docente.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/22_admin_management_students.png}
    \caption{Gestión y directorio general de estudiantes.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/23_admin_management_teachers.png}
    \caption{Mantenimiento y directorio del personal docente.}
\end{figure}

\subsubsection{Gestión de cursos y períodos académicos}

Estas pantallas permiten estructurar la oferta académica, definiendo los ciclos lectivos y los cursos habilitados en cada uno de ellos para la correcta operación del sistema durante la matrícula.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/24_admin_management_courses.png}
    \caption{Gestión del catálogo general de cursos.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Entregable 18/25_admin_management_periods.png}
    \caption{Administración de los periodos académicos institucionales.}
\end{figure}
...
El Panel ejecutivo ha sido meticulosamente diseñado para proporcionar una vista consolidada e interactiva del desempeño del proyecto \textit{Plataforma Universitaria Integrada} a lo largo de todo su ciclo de vida, operando como una herramienta analítica integral que permite visualizar de forma centralizada los principales indicadores clave de desempeño (KPIs) con el fin de facilitar la toma de decisiones estratégicas por parte de la dirección y el patrocinador.

Para lograr este objetivo, el documento se encuentra estructurado en diversas hojas temáticas que abordan los aspectos más críticos de la gestión —tales como el avance general, el cumplimiento riguroso del cronograma, el estado detallado del presupuesto y el monitoreo continuo de los riesgos—; adicionalmente, es importante destacar que el archivo original, el cual es completamente interactivo y funcional, ha sido puesto a disposición de los interesados bajo el nombre \textbf{Entregable 23 - Dashboard ejecutivo.xlsx}, y a continuación se exponen las capturas de pantalla que documentan las vistas principales de este panel.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Entregable 23 - Dashboard ejecutivo/01.png}
\caption{Dashboard principal y resumen de KPIs}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Entregable 23 - Dashboard ejecutivo/02.png}
\caption{Desglose y desempeño del presupuesto}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Entregable 23 - Dashboard ejecutivo/03.png}
\caption{Cronograma general y control de hitos}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Entregable 23 - Dashboard ejecutivo/04.png}
\caption{Monitoreo integral y matriz de riesgos}
\end{figure}
...
El presente documento constituye el informe ejecutivo del proyecto Plataforma Universitaria Integrada, el cual fue diseñado con el propósito principal de consolidar y presentar los resultados finales alcanzados tras la finalización del proyecto.
...
\begin{tabularx}{\textwidth}{Y Y Y}
\toprule
\textbf{{Aprobado por} & \textbf{{Aprobado por} & \textbf{Aprobado por} & \textbf{Aprobado por}& \textbf{Aprobado por} & \textbf{Aprobado por} & \textbf{Aprobado por}\\
\midrule
& & \\
& & \\
Lic. Patricia Vargas Soto & Tamara Robles Camacho & Mauricio González Prendas & Carlos Sainz Arce & Isaac Jiménez Blanco & Jorge Abarca Quiros  & Dr. Roberto Mora Fallas  \\
Directora Administrativa & Project Manager & Desarrollador Frontend 1 y QA & Desarrollador Frontend 2 y QA  & Analista de Negocio & Documentador 1 & Analista de Negocio & Documentador 2 & Sponsor  \\
\bottomrule
\end{tabularx}
\end{table}
\end{document}
