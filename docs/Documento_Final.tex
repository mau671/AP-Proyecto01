...
El presente entregable corresponde al cronograma del proyecto \textit{Plataforma Universitaria Integrada}, elaborado mediante Microsoft Project. Su propósito es organizar las actividades del proyecto, definir la secuencia lógica del trabajo, asignar responsables, estimar duraciones, establecer dependencias, controlar costos y visualizar los hitos principales. Además, el cronograma permite identificar la ruta crítica, la cual representa la secuencia de actividades que determina la duración total del proyecto.

El archivo editable del cronograma se conserva en formato \texttt{.mpp}, mientras que la versión visual exportada desde Microsoft Project se encuentra disponible en el repositorio del proyecto con el nombre \texttt{cronograma.pdf}. Esta evidencia externa permite revisar de forma completa el Diagrama de Gantt, las barras del calendario, las fechas, las dependencias y la ruta crítica sin afectar la legibilidad del presente documento final.

\subsection{Información general del cronograma}

La configuración general del cronograma se resume en la siguiente tabla.

\begin{table}[H]
\centering
\caption{Configuración general del cronograma del proyecto}
{\small
...
Fecha de inicio planificada & 8 de junio de 2026 \
Fecha final planificada & 9 de setiembre de 2026 \
Duración aproximada & Tres meses \
Presupuesto total & ₡17,625,200 \
Método de programación & Tareas programadas automáticamente \
Calendario utilizado & Calendario estándar \
Evidencia visual & Archivo \texttt{cronograma.pdf} ubicado en el repositorio del proyecto \
\bottomrule
\end{tabularx}
}
\end{table}

\subsection{Estructura general del cronograma}

El cronograma fue organizado por fases con el fin de separar las actividades de inicio, planificación, ejecución, supervisión y control, cierre, y rubros adicionales. Esta estructura permite relacionar el trabajo del proyecto con los grupos de procesos vistos en el curso y facilita el seguimiento del avance.

\begin{table}[H]
\centering
\caption{Resumen de fases principales del cronograma}
{\small
\begin{tabularx}{\textwidth}{L{3.6cm}Y R{3cm}}
\toprule
\rowcolor{teclightblue}
\textbf{Fase} & \textbf{Descripción} & \textbf{Costo} \
\midrule
Inicio & Incluye la aprobación del Business Case, la elaboración del Project Charter y el registro de stakeholders. & ₡1,886,667 \
\midrule
Planificación & Incluye definición del alcance, WBS, cronograma, matriz RACI, organigrama, plan de recursos, costos, calidad, comunicaciones, riesgos y adquisiciones. & ₡3,686,667 \
\midrule
...
\midrule
Cierre & Incluye informe ejecutivo, acta de cierre, reflexiones, recomendaciones y revisión final de la entrega. & ₡588,333 \
\midrule
Rubros adicionales y contingencia & Incluye software, hardware, servicios operativos y reserva para contingencias. & ₡2,181,866 \
\midrule
\textbf{Total} & \textbf{Presupuesto total planificado del proyecto.} & \textbf{₡17,625,200} \
\bottomrule
\end{tabularx}
}
\end{table}

\subsection{Verificación de elementos solicitados}

El Entregable 06 cumple con los elementos requeridos para el cronograma del proyecto, ya que fue construido en Microsoft Project e incluye tareas, subtareas, dependencias, duraciones, recursos, hitos, ruta crítica, responsables y costos.

\begin{table}[H]
\centering
\caption{Verificación de cumplimiento del Entregable 06}
{\small
\begin{tabularx}{\textwidth}{L{4cm}Y}
\toprule
\rowcolor{teclightblue}
...
\midrule
Tareas & Se identifican mediante las actividades individuales del Diagrama de Gantt, tales como elaborar el Project Charter, definir el alcance, crear el cronograma, desarrollar módulos y realizar pruebas. \
\midrule
Subtareas & Se reflejan en las actividades agrupadas dentro de fases como Inicio, Planificación, Ejecución, Supervisión y control, Cierre y Rubros adicionales. \
\midrule
Dependencias & Se representan mediante la columna de predecesoras, la cual define qué actividades deben finalizar antes de que otras puedan iniciar. \
\midrule
Duraciones & Cada tarea cuenta con una duración estimada en días, calculada dentro de Microsoft Project. \
\midrule
Recursos & Se asignaron recursos humanos y recursos de costo en la hoja de recursos del proyecto. \
\midrule
Hitos & Se incluyeron tareas de duración cero para señalar puntos clave de aprobación o cierre. \
\midrule
Ruta crítica & Se identificó la secuencia de tareas que determina la fecha final del proyecto. \
\midrule
Responsables & Se asignaron responsables mediante los recursos vinculados a cada tarea. \
\midrule
Costos & Se registraron costos fijos por actividad y costos acumulados por fase, manteniendo el presupuesto total de ₡17,625,200. \
\bottomrule
\end{tabularx}
}
\end{table}
...
En Microsoft Project, las tareas resumen representan fases completas del proyecto. Por ejemplo, Inicio, Planificación, Ejecución, Supervisión y control, Cierre, y Rubros adicionales agrupan actividades específicas. Las tareas normales poseen duración, fechas de inicio y fin, responsables, costos y predecesoras.

Los hitos se utilizan para marcar puntos relevantes del proyecto sin agregar duración adicional al cronograma. En este caso, los principales hitos considerados son los siguientes:
...
Estos hitos permiten verificar avances importantes y funcionan como puntos de control dentro de la planificación general.

\subsection{Dependencias y predecesoras}

Las dependencias permiten que el cronograma tenga una secuencia lógica. En lugar de ser una lista aislada de actividades, el cronograma muestra qué tareas deben completarse antes de iniciar otras. Por ejemplo, la definición del alcance debe estar lista antes de avanzar con varias tareas de planificación y con el desarrollo del prototipo. De igual forma, el cierre depende de la finalización del prototipo y de las actividades de supervisión y control.

En Microsoft Project, estas relaciones se registran mediante la columna de predecesoras. Si una actividad relacionada se atrasa, la herramienta permite recalcular las fechas posteriores y visualizar el efecto sobre la planificación general del proyecto.

\subsection{Ruta crítica}

La ruta crítica corresponde a la secuencia de actividades que determina la duración total del proyecto. Si una actividad perteneciente a esta ruta se atrasa y no se recupera tiempo en otra actividad posterior, la fecha final del proyecto también puede verse afectada.

Con base en el cronograma elaborado, la ruta crítica principal se concentra en la definición del alcance, el desarrollo secuencial del prototipo Front-End, las pruebas de calidad y el cierre del proyecto. La secuencia crítica principal es la siguiente:
...
Esta ruta se considera prioritaria para el seguimiento, ya que concentra las actividades que controlan directamente la fecha final del proyecto. Aunque existen tareas de planificación y supervisión que se ejecutan en paralelo, varias de ellas finalizan antes de las actividades críticas de desarrollo y cierre.

\subsection{Recursos y responsables}

La hoja de recursos del cronograma incluye recursos humanos y recursos de costo. Los recursos humanos representan a los integrantes responsables de ejecutar o coordinar actividades. Los recursos de costo representan rubros económicos asociados al proyecto, tales como software, infraestructura, materiales, servicios y contingencia.

\begin{table}[H]
\centering
\caption{Recursos humanos principales asignados al cronograma}
{\small
\begin{tabularx}{\textwidth}{L{5cm}Y L{3.5cm}}
\toprule
\rowcolor{teclightblue}
\textbf{Recurso} & \textbf{Rol dentro del proyecto} & \textbf{Grupo} \
\midrule
...
\subsection{Costos del cronograma}

El cronograma utiliza costos fijos por tarea para evitar duplicar costos por horas de recursos. De esta forma, las tareas resumen acumulan los costos de sus actividades internas y permiten revisar el presupuesto por fase. El costo total planificado del proyecto es de ₡17,625,200.

La fase con mayor costo es Ejecución, ya que contiene el desarrollo del prototipo Front-End navegable, la integración entre módulos y las pruebas de usabilidad y QA. Asimismo, el cronograma incluye una reserva para contingencias, la cual funciona como margen económico ante imprevistos menores del proyecto.

\subsection{Evidencia visual del cronograma}

Para evitar problemas de legibilidad dentro del documento final, el Diagrama de Gantt visual no se incrusta directamente en esta sección. La evidencia completa del cronograma se encuentra en el repositorio del proyecto con el nombre \texttt{cronograma.pdf}. Este archivo muestra la vista exportada desde Microsoft Project, incluyendo las barras del calendario, tareas, fechas, responsables, dependencias y ruta crítica.

Adicionalmente, el archivo editable del cronograma se conserva en formato \texttt{.mpp}, lo cual permite abrirlo en Microsoft Project para consultar o ajustar la planificación original si fuera necesario.

\subsection{Relación con la materia}

Este entregable se relaciona directamente con los contenidos de administración de proyectos, ya que integra planificación del tiempo, asignación de recursos, estimación de costos, dependencias, hitos y ruta crítica. También funciona como una base para el monitoreo y control, debido a que permite comparar el avance real contra lo planificado y detectar desviaciones en fechas, costos o alcance.

Por esta razón, el cronograma no se interpreta únicamente como un calendario, sino como una herramienta de gestión que permite organizar, controlar y comunicar el avance del proyecto de forma estructurada.
