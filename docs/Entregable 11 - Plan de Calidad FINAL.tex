...
De acuerdo con el caso de negocio del proyecto, la plataforma busca ofrecer una solución integrada que permita representar de manera clara los procesos universitarios principales y reducir los problemas asociados a la fragmentación de la información. Sin embargo, dentro del alcance definido para esta entrega no se desarrollará un sistema completo en producción, sino un prototipo de interfaz web navegable. Por esta razón, la calidad del proyecto debe evaluarse de forma coherente con ese alcance, considerando principalmente la navegación, la representación visual de los módulos, la consistencia de la interfaz, la claridad de la información presentada y la alineación entre el prototipo y la documentación administrativa del proyecto.

El plan de calidad permite definir cómo se asegurará que los entregables cumplan con los requisitos establecidos desde la fase de inicio y planificación. Para ello, se toman como referencia los documentos y definiciones organizacionales elaboradas previamente, las cuales establecen elementos esenciales como el propósito del proyecto, los interesados principales, los roles del equipo, el alcance incluido, el alcance excluido, las restricciones, las fechas de trabajo, los costos estimados y los criterios generales de aceptación. Por lo tanto, este plan no se limita a revisar el prototipo, sino que también busca mantener la coherencia a lo largo de toda la iniciativa.
...
Garantizar que el prototipo de interfaz web navegable represente correctamente los módulos principales definidos en el alcance. Cada módulo deberá permitir una navegación clara, el uso de datos simulados y la visualización de la información mínima establecida para cada portal correspondiente.

\paragraph*{Objetivo 2}\mbox{}\\
Asegurar que todos los entregables documentales mantengan una coherencia transversal con la documentación estratégica previamente aprobada. Esto incluye mantener los mismos nombres de roles, responsables, fechas, presupuesto total, alcance incluido, alcance excluido y criterios generales de aceptación en toda la documentación elaborada.
...
\begin{itemize}[leftmargin=*]
    ...
    \item El presupuesto deberá mantenerse alineado con el caso de negocio.
...
\paragraph*{Buenas prácticas de desarrollo de la interfaz}
...
Las métricas permiten verificar si el proyecto cumple con los objetivos definidos. Para este proyecto se utilizarán métricas simples, debido a que el alcance corresponde a un prototipo académico de interfaz navegable y no a un sistema en producción.
...
Evidencia & Capturas, archivos y documentos organizados & Repositorio final completo en GitHub \\
\bottomrule
\end{tabularx}
}
\end{table}
...
\begin{itemize}[leftmargin=*]
    ...
    \item GitHub para almacenar y compartir el repositorio final del proyecto.
...
\paragraph*{Desarrollador de interfaz 1 y control de calidad}\mbox{}\\
...
\paragraph*{Desarrollador de interfaz 2 y control de calidad}\mbox{}\\
Carlos Sainz Arce
...
\paragraph*{Interesados externos de validación}\mbox{}\\
...
El plan de calidad se vincula de manera directa con el cronograma general del proyecto. Dentro de dicha planificación se ha incluido la tarea correspondiente para elaborar el plan de calidad, además de diversas actividades posteriores orientadas al desarrollo de la interfaz, el control de cambios, la evaluación técnica y la revisión integral de cierre.

La calidad guarda igualmente una estrecha relación con el presupuesto del proyecto. En la estimación de costos actualizada se definió un presupuesto total estimado de ₡17,625,200. Dentro de dicho monto se contempla el esfuerzo de pruebas de usabilidad y control de calidad del prototipo. Adicionalmente, diversas labores de calidad se encuentran distribuidas dentro del esfuerzo de documentación, desarrollo, gestión y verificación técnica.

El control de calidad será especialmente importante durante la etapa final del proyecto, ya que el cronograma debe mantenerse controlado y la entrega debe realizarse según la fecha definida en Microsoft Project. Por esta razón, el equipo debe priorizar la prevención de errores, la revisión temprana y la validación continua de los módulos principales. Esto reduce el riesgo de retrabajo y ayuda a mantener la calidad sin afectar el cronograma.

También resulta pertinente considerar que la iniciativa presenta un alcance estrictamente académico y delimitado. Por tal motivo, no se realizarán pruebas de lógica de servidor, pruebas de bases de datos, simulaciones de carga reales, auditorías de seguridad avanzadas ni validaciones integradas con sistemas institucionales en producción. La calidad del trabajo será evaluada conforme a los parámetros formalmente definidos en el alcance de forma que se valore el prototipo de interfaz navegable, la documentación coherente, la gestión correcta de procesos y la demostración objetiva de los logros alcanzados.

\section{Conclusión}

El plan de calidad del proyecto Plataforma Universitaria Integrada instituye de forma clara los criterios, normas, estándares, métricas, actividades, herramientas y responsabilidades requeridas para garantizar el cumplimiento pleno de los parámetros establecidos. Este documento de control resulta determinante para examinar no solo el comportamiento técnico del producto final, sino también el nivel de excelencia de los procesos gerenciales aplicados por el equipo de trabajo.

De esta manera, el aseguramiento del producto técnico se evaluará primordialmente mediante inspecciones continuas del prototipo de interfaz web navegable, con particular interés en los flujos de interacción, la integración de los módulos, la homogeneidad del diseño, la simulación de datos y la eliminación de deficiencias estructurales. Por su parte, la calidad administrativa del proceso se validará a través de la exhaustiva revisión de la coherencia documental, la verificación del cronograma, el control de cambios y el correcto resguardo de las evidencias formales.

Asimismo, la correcta implementación de estas directrices de calidad asiste de forma activa en el mantenimiento de las restricciones de alcance, ya que restringe la integración extemporánea de funciones excluidas tales como bases de datos reales, esquemas de autenticación institucionales, pasarelas de pago y despliegues productivos. En consecuencia, esta delimitación permite que el equipo concentre sus esfuerzos exclusivamente en la elaboración de los componentes mandatarios para generar así un entregable sumamente consolidado y plenamente ajustado a los objetivos pactados.

En conclusión, la estructuración y puesta en marcha de estas prácticas preventivas y de inspección otorgan al equipo una guía idónea para neutralizar problemas, subsanar deficiencias de código antes de la liberación final y corroborar que el resultado de la iniciativa satisfaga las expectativas documentadas formalmente a través de todos los entregables aprobados durante la etapa previa de planificación general.
